...
class PointServiceFeed {
    async feed(req, res) {
        ...
        NUMERO_OPERACAO = String(Number(String(NUMERO_OPERACAO).replaceAll(' ', '')));
        ...
        if (CODIGO_PECA.includes("RE.")) {
            ...
            resource = await (0, query_1.select)(VW_FEED_ODF, {
                NUMERO_ODF,
                NUMERO_OPERACAO,
                CODIGO_MAQUINA,
                REVISAO,
        ...
        if (NUMERO_OPERACAO === lastProcessOnOdf) {
            ...
            const address = await connection
                .query(`SELECT QUANTIDADE FROM CST_ESTOQUE_ENDERECOS WHERE 1 = 1 AND ODF = '${NUMERO_ODF}'`)

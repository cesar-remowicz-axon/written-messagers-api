...
const selectIfHasP_1 = require("../../utils/selectIfHasP");
...
class OdfServiceToInit {
    async init(req, res) {
        ...
        if (ESTOQUE_MOD === "TRUE") {
            ...
            if (components["message"] === (0, message_1.message)("NoLimit") || Number(components["quantidade"]) <= 0) {
                apiResponse["message"] = `Sem quantidade para apontamento : ${components["semLimite"]}`;
                return res.status(200).json(apiResponse);
            }
            if (components["message"] !== (0, message_1.message)('Success')) {
            ...
            apiResponse['childrenAddress'] = (components['childrenAddress']);
        ...
        if (!code || code === (0, message_1.message)("RipFin") || code === (0, message_1.message)("Return")) {
            apiResponse["code"] = (0, message_1.message)("SetupIni");
            if (FERRAMENTA_MOD === "TRUE" && mode !== 'Demo') {
...
exports.default = OdfServiceToInit;
